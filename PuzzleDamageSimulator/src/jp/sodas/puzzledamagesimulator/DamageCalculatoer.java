...
public class DamageCalculatoer {
  ...
  public double BonusOfDrops(int drops, int enhancedDrops, int arousal) {
    ...
    if (enhancedDrops > 0) {
      bonusOfDrops = bonusOfDrops * (arousal * Arousal.MAGNIFICATION_OF_ENHANCEDROP + 1.00);// ドロップ強化の覚醒による倍率
    }
    return bonusOfDrops;
...
//        damage += target.getAttack() * bonusOf2way(arousals) * BonusOfDrops(drops, enhancedDrops, arousal);
//      }
//    }
